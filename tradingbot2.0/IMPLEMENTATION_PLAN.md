...
- **Root Cause**: Random temp filenames generated by `tempfile.NamedTemporaryFile()` could accidentally contain "1s", "1h", or "1d" patterns, causing schema detection logic to pick the wrong schema based on filename pattern matching
- **Fix**: Added explicit `prefix="test_default_"` parameter to `tempfile.NamedTemporaryFile()` to ensure deterministic filename that doesn't match any schema detection patterns
- **Impact**: Test now passes consistently without race conditions or flakiness
